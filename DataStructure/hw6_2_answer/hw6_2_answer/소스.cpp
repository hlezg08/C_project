...
void huffman_traversal(TreeNode *node)
{
	...
	if (node != NULL)
	{
		...
		{
			char data = node->data;
			int bit_size = node->bit_size;
			int *bits_tp = node->bits;
			switch (data) {
			case 'a':
				m_LUT[0] = bits_tp;
				m_bit_size[0] = bit_size;
				break;
			case 'b':
				m_LUT[1] = bits_tp;
				m_bit_size[1] = bit_size;
				break;
			case 'c':
				m_LUT[2] = bits_tp;
				m_bit_size[2] = bit_size;
				break;
			case 'd':
				m_LUT[3] = bits_tp;
				m_bit_size[3] = bit_size;
				break;
			case 'e':
				m_LUT[4] = bits_tp;
				m_bit_size[4] = bit_size;
				break;
			case 'f':
				m_LUT[5] = bits_tp;
				m_bit_size[5] = bit_size;
				break;
			}
		}
		else
		{
			//Left child <- 0
			bit_index = node->bit_size;
			memcpy(node->l->bits, node->bits, sizeof(int)*MAX_BIT);
			node->l->bits[bit_index] = 0;
			node->l->bit_size = bit_index + 1;
			huffman_traversal(node->l);
			//Right child <- 1
			bit_index = node->bit_size;
			memcpy(node->r->bits, node->bits, sizeof(int)*MAX_BIT);
			node->r->bits[bit_index] = 1;
			node->r->bit_size = bit_index + 1;
