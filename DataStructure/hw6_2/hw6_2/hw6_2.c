...
{	
	...
	if (node->l == NULL && node->r == NULL) {
		for (i = 0; i < m_char_size; i++) {
			for (j = 0; j < m_bit_size[i]; j++) {
				m_LUT[i][j] = node->bits[i];
			}
		}
	}
...
void huffman_encoding(char *str, bits_stream *bits_str)
{
	for (int i = 0; i < strlen(str); i++) {
		for (int j = 0; j < m_bit_size[i]; j++) {
			if (str[i] == 'a') {
				bits_str->stream[i] += m_LUT[0][j];
			}
			if (str[i] == 'b') {
				bits_str->stream[i] += m_LUT[1][j];
			}
			if (str[i] == 'c') {
				bits_str->stream[i] += m_LUT[2][j];
			}
			if (str[i] == 'd') {
				bits_str->stream[i] += m_LUT[3][j];
			}
			if (str[i] == 'e') {
				bits_str->stream[i] += m_LUT[4][j];
			}
			if (str[i] == 'f') {
				bits_str->stream[i] += m_LUT[5][j];
			}
		}
	}
	int interval = 0;
	for (int i = 0; i < strlen(bits_str); i++) {
		if (bits_str->stream[i] != NULL)
			interval++;
	}
	printf("\n* Huffman encoding\n");
