...
void main()
{
	int service_timeA = 0,service_timeB=0; //server가 두명이니까 각각 다른 변수 설정
	...
	while (clock < duration) {
		...
		if (service_timeA > 0)	// the customer is receiving service
			service_timeA--;

		// no customer is receiving service. 
		// So, take out a customer from the queue and start the service.
		else {
			service_timeA = remove_customer();
		}
		if (service_timeB > 0)	//service_timeA와 조건문 동일
			service_timeB--;

		else {
			service_timeB = remove_customer();
		}
	}
	print_stat();
}
